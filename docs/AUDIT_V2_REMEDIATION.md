...
### موجة 5 — P3
**بنود الصحّة/الأمان اتعملت (مش تجميلية):**
- [x] تعليم `fifo_layers` + `item_balances` الميتة.
- [x] تصادم الكوبون (TOCTOU) — العدّاد الذرّي في `incrementRedemption` (WHERE على السقف + RETURNING).
- [x] self-target التجاوزات — `setMemberOverridesAction` يرفض `userId === auth.userId` (تصعيد صلاحيات).
- [x] يتم مستندات المسودّة عند حذف الأمر — اتغطّى في **#7** (حذف الفاتورة المسودّة يعيد فتح الأمر).

**بنود تجميلية/IA بحتة → مدمجة في [هدف إعادة الهيكلة/الـIA](../CLAUDE.md) (باكلوج الهدف ده، مش تُلاحَق منفصلة عشان الريستركتشر هيعيد صياغة نفس الأسطح):**
- [ ] skeletons تحميل لكل قسم ثقيل · «مسودات» كعنصر ناف دائم.
- [ ] توحيد/ربط شاشات aging الثلاث · كروت هوم الأدمن الناقصة · إحصائية «طلبات معلّقة» قابلة للنقر.
- [ ] تلميحة الرصيد تفرز بـ`number` · ليبل EXPIRED على شاشات المخزون.
...
## 5. قرارات موجة 4 — **كلها اتحسمت ونُفّذت** (بالافتراضات المحافِظة)

> ملخّص التنفيذ: SOFT_CLOSED = يمنع الترحيل (#10) · مفاتيح API = scope+expiry اختياري، القديم write/دائم (#16) · الإجازة بدون أجر = proration بأيام تقويمية (#18) · rate-limit = قفل لكل حساب DB-backed 10 محاولات/15 دقيقة (#15) · الحجز = DRAFT يحجز (الأأمن، موثّق) · #5b = قراءة مقيّدة + عمود org مؤجّل (جدول dormant).

### (الأصل) القرارات المطروحة
