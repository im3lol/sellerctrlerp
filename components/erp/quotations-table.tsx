...
export function QuotationsTable({ rows, canDelete }: { rows: Row[]; canDelete: boolean }) {
          ...
          {rows.map((r) => {
            ...
            return (
              <TableRow key={r.id} data-state={sel.has(r.id) ? "selected" : undefined}>
                ...
                {canDelete && <TableCell><QuotationRowActions id={r.id} status={r.status} canManage={canDelete} /></TableCell>}
              </TableRow>
            );
          })}
        </TableBody>
