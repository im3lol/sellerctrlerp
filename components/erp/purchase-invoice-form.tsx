...
export function PurchaseInvoiceForm({ suppliers, warehouses, items }: { suppliers: Supplier[]; warehouses: Warehouse[]; items: Item[] }) {
  ...
  // Add the scanned/searched item: bump quantity if present, else fill an empty
  // line or append. (Purchase price is entered manually — left at 0.)
  const addOrBumpItem = (item: ItemSearchResult) =>
    setLines((ls) => {
      ...
      const line = { itemId: item.id, quantity: 1, unitPrice: 0, discountAmount: 0, taxAmount: 0 };
  ...
  const submit = () => {
    ...
    start(async () => {
      const r = await createPurchaseInvoiceAction({ supplierId, warehouseId, date, notes, lines });
      if (r.ok) { toast.success("تم إنشاء الفاتورة"); router.push("/erp/purchases/invoices"); router.refresh(); }
      ...
      <CardContent className="space-y-5">
        ...
        <div className="grid gap-4 rounded-xl border bg-muted/30 p-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <BarcodeScan onScan={addOrBumpItem} />
          </div>
          <div className="space-y-2">
            <Label>بحث وإضافة صنف</Label>
            <ItemCombobox onSelect={addOrBumpItem} />
          </div>
        </div>
            ...
            <TableBody>
              {lines.map((l, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell><Button variant="ghost" size="icon" onClick={() => removeLine(i)} aria-label="حذف البند"><Trash2 className="size-4 text-destructive" /></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
        ...
        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={() => router.push("/erp/purchases/invoices")}>إلغاء</Button>
          <Button onClick={submit} disabled={pending}>{pending && <Loader2 className="size-4 animate-spin" />}حفظ الفاتورة</Button>
        </div>
      </CardContent>
    </Card>
  );
