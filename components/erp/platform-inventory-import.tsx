...
export function PlatformInventoryImport({ platformId, platformName, hasWarehouse }: { platformId: string; platformName: string; hasWarehouse: boolean }) {
  ...
  const apply = () => {
    ...
    startApply(async () => {
      ...
      if (r.ok) { toast.success("تم إنشاء تسوية مخزون (مسودة) — أكّدها لترحيلها"); router.push(r.number ? `/inventory/adjustments/${encodeURIComponent(r.number)}` : "/inventory/adjustments"); router.refresh(); }
