...
export function PostPurchaseInvoiceButton({ id }: { id: string }) {
  const router = useRouter();
  const [pending, start] = useTransition();
  return (
    <Button size="sm" variant="outline" disabled={pending}
      onClick={() => start(async () => {
        const r = await postPurchaseInvoiceAction(id);
        if (r.ok) { toast.success("تم ترحيل الفاتورة محاسبياً"); router.refresh(); }
