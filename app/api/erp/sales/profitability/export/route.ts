...
export async function GET(req: Request) {
  ...
  return xlsxResponse({
    ...
    headers: ["الكود", "الصنف", "الكمية", "الإيراد", "التكلفة", "الربح", "الهامش %"],
    rows: list.map((r) => [r.code, r.name, r.qty, r.revenue, r.cogs, r.profit, r.revenue > 0 ? Number(r.margin.toFixed(1)) : ""]),
    totalRow: ["", "الإجمالي", "", tRev, tCogs, tRev - tCogs, ""],
    colWidths: [12, 28, 12, 16, 16, 16, 10],
  });
  });
}
