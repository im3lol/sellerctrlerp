...
export const ERP_ROLES = ["admin", "accountant", "inventory", "sales", "purchases", "viewer"] as const;
...
export async function addUserToOrgAction(userId: string, role: string): Promise<ActionState> {
  ...
  if (!(ERP_ROLES as readonly string[]).includes(role)) return { error: "دور غير صالح" };
