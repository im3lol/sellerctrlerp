...
async function nextNumber(orgId: string, year: number): Promise<string> {
  ...
  const [last] = await db.select({ number: stockAdjustments.number }).from(stockAdjustments)
    .where(and(eq(stockAdjustments.organizationId, orgId), like(stockAdjustments.number, `${prefix}%`)))
    .orderBy(desc(stockAdjustments.number)).limit(1);
  let seq = 1;
