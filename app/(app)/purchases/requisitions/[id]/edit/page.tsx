...
export default async function EditMaterialRequestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return loadErpPage("purchases.create", async ({ orgId }) => {
    ...
    if (!mr) notFound();
    if (mr.status !== "DRAFT") redirect(`/purchases/requisitions/${mr.id}`);
    ...
    const initial: MaterialRequestInitial = {
      id: mr.id, date: new Date(mr.date).toISOString().slice(0, 10), notes: mr.notes ?? "",
      lines: mrLines.map((l) => ({ itemId: l.itemId, quantity: Number(l.quantity) || 0 })),
    };

    return (
      <div className="space-y-6">
        <ErpPageHeader icon="ClipboardList" title={`تعديل طلب مواد ${mr.number}`} subtitle="مسودة — عدّل الأصناف والكميات ثم احفظ" backHref={`/purchases/requisitions/${mr.id}`} />
