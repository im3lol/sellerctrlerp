...
export default async function PurchaseReturnDetailPage({ params }: { params: Promise<{ number: string }> }) {
  ...
  return loadErpPage("purchases.view", async ({ orgId, can }) => {
    if (UUID_RE.test(raw)) {
      ...
      if (!byId) notFound();
      redirect(`/purchases/returns/${encodeURIComponent(byId.number)}`);
    }
    ...
    const [[sup], lines, [pi], [grn], audit] = await Promise.all([
      ret.supplierId
        ? db.select({ code: suppliers.code, name: suppliers.nameAr }).from(suppliers).where(eq(suppliers.id, ret.supplierId)).limit(1)
        : Promise.resolve([undefined] as { code: string; name: string }[] | [undefined]),
      db.select({ id: purchaseReturnLines.id, qty: purchaseReturnLines.quantity, unitPrice: purchaseReturnLines.unitPrice, total: purchaseReturnLines.totalAmount, code: items.code, name: items.nameAr })
        .from(purchaseReturnLines).leftJoin(items, eq(items.id, purchaseReturnLines.itemId)).where(eq(purchaseReturnLines.purchaseReturnId, ret.id)),
    ...
    let backHref = "/purchases/invoices";
    if (pi) linked.push({ label: "فاتورة شراء", number: pi.number, href: `/purchases/invoices/${encodeURIComponent(pi.number)}` });
    if (grn) { const href = `/purchases/receipts/${encodeURIComponent(grn.number)}`; linked.push({ label: "إذن استلام", number: grn.number, href }); backHref = href; }
    ...
    return (
      <div className="space-y-6">
        ...
        <DocChatter kind="PURCHASE_RETURN" entityId={ret.id} entityNumber={ret.number} audit={audit} />
      </div>
    );
  });
}
