...
export default async function SalesInvoiceDetailPage({ params }: { params: Promise<{ number: string }> }) {
  ...
  return loadErpPage("sales.view", async ({ orgId, role, can }) => {
    if (UUID_RE.test(raw)) {
      ...
      if (!byId) notFound();
      redirect(`/sales/invoices/${encodeURIComponent(byId.number)}`);
    }
    ...
    if (dn) linked.push({ label: "إذن صرف", number: dn.number, href: `/sales/deliveries/${encodeURIComponent(dn.number)}` });
