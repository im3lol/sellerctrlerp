  ...
  return loadErpPage("hr.view", async ({ orgId, can }) => {
    ...
    return (
      <div className="space-y-6">
        <ErpPageHeader icon="CalendarDays" title="إجازات الموظفين" subtitle={`${rows.length} طلب`} backHref="/hr"
          action={
            <div className="flex gap-2">
              <Button asChild variant="outline"><Link href="/hr/leaves/report"><Icon name="BarChart3" className="size-4" />التقرير</Link></Button>
              {can("hr.create") && <Button asChild><Link href="/hr/leaves/new"><Icon name="Plus" className="size-4" />طلب إجازة</Link></Button>}
            </div>
          } />
        <Card>
