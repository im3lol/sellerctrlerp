...
export default async function NewFixedAssetPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  return loadErpPage("accounting.create", async ({ orgId }) => {
    ...
    async function create(fd: FormData) {
      ...
      redirect(`/accounting/assets/new?error=${encodeURIComponent(res.error ?? "تعذّر حفظ الأصل")}`);
    }

    return (
      <div className="space-y-6" dir="rtl">
        <ErpPageHeader icon="Building2" title="أصل ثابت جديد" backHref="/accounting/assets" />
            ...
            <form action={create} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button type="button" variant="ghost" asChild>
                  <Link href="/accounting/assets">إلغاء</Link>
                </Button>
              </div>
            </form>
          </CardContent>
