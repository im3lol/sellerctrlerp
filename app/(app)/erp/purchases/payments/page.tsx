                ...
                <TableBody>
                  {rows.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="font-mono">{r.invoice ? <Link href={`/erp/purchases/invoices/${encodeURIComponent(r.invoice)}`} className="text-primary hover:underline">{r.invoice}</Link> : "تحت الحساب"}</TableCell>
