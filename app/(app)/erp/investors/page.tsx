...
const SHORTCUTS = [
  { label: "المستثمرون", href: "/erp/investors/list", icon: "Coins", key: "investors" },
  { label: "مساهمات رأس المال", href: "/erp/investors/investments", icon: "PiggyBank", key: "investments" },
  { label: "توزيعات الأرباح", href: "/erp/investors/distributions", icon: "PieChart", key: "distributions" },
  { label: "السحوبات", href: "/erp/investors/withdrawals", icon: "Banknote" },
...
export default async function InvestorsPage() {
  const { orgId } = await requireErpModule("investors.view");
  ...
  const todos = [
    { label: "توزيعات أرباح مسودة", hint: "بانتظار الترحيل", count: cnt(distDraft), href: "/erp/investors/distributions", icon: "PieChart" },
  ];
          ...
          <CardContent>
            {owners.length === 0 ? (
              <p className="py-8 text-center text-sm text-muted-foreground">
                ...
                <Link href="/erp/investors/investments" className="text-primary underline">مساهمات رأس المال</Link>.
              </p>
            ) : (
