...
export default async function SalesReturnDetailPage({ params }: { params: Promise<{ number: string }> }) {
  ...
  if (ret.salesInvoiceId) {
    ...
    if (si) linked.push({ label: "الفاتورة الأصلية", number: si.number, href: `/erp/sales/invoices/${encodeURIComponent(si.number)}` });
  }
  ...
  return (
    <div className="space-y-6">
      <ErpPageHeader
        ...
        subtitle={cust ? `${cust.code} — ${cust.name}` : "مرتجع مبيعات (إشعار دائن)"}
        backHref="/erp/sales/returns"
        action={<ReturnRowActions returnId={ret.id} type="sales" status={ret.status} canManage={canManage} />}
      />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Field label="النوع">إشعار دائن</Field>
      </div>

      <Card>
        <CardHeader><CardTitle>البنود المرتجعة</CardTitle><CardDescription>البضاعة العائدة للمخزون.</CardDescription></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
