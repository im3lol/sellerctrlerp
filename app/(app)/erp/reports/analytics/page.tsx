  ...
  return loadErpPage("reports.view", async ({ orgId }) => {
    ...
    const alerts = ov
      ? ([
          ...
          ov.outOfStock > 0 && { label: `أصناف نافدة: ${int(ov.outOfStock)}`, href: "/erp/inventory/reorder", danger: true },
          ov.lowStock > 0 && { label: `مخزون منخفض: ${int(ov.lowStock)}`, href: "/erp/inventory/reorder", danger: false },
          ov.nearExpiryCount > 0 && { label: `قرب انتهاء الصلاحية: ${int(ov.nearExpiryCount)}`, href: "/erp/inventory/expiry", danger: false },
